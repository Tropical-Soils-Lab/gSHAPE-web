...
def fetch_climate(lat, lon, need_precip=False):
    try:
        ...
        if need_precip:
            ...
            if map_v is not None: result["precip"] = float(map_v) * 365.25
...
def get_params_1d(df, tax, tex, target_temp):
    sub = df[(df["peer_group_taxon"] == tax) & (df["peer_group_texture"] == tex)].sort_values("PRISM_tmea").reset_index(drop=True)
    ...
    if lo.empty or hi.empty: return sub.iloc[(sub["PRISM_tmea"] - tc).abs().argsort().iloc[0]]
...
def get_params_any(cfg, df, tax, tex, temp, precip=None):
    if "precip" in cfg["predictors"]: return get_params_2d(df, tax, tex, temp, precip)
    return get_params_1d(df, tax, tex, temp)

def logit(x): return np.log(np.clip(x, 0.0001, 0.9999) / (1 - np.clip(x, 0.0001, 0.9999)))
def invlogit(x): return 1 / (1 + np.exp(-x))
def compute_score(oc, lp_mean, sigma_val): return float(norm.cdf(logit(np.array(oc) / 100), loc=lp_mean, scale=sigma_val) * 100)

def score_color(s):
    if s >= 80: return "#1a9641"
    if s >= 60: return "#77c35c"
    if s >= 40: return "#ffc107"
    if s >= 20: return "#f46d43"
    return "#d73027"
...
def percentile_to_oc(pct, lp_mean, sigma_val): return invlogit(norm.ppf(pct / 100, loc=lp_mean, scale=sigma_val)) * 100
def parse_code(display_str): return display_str.split("(")[1].replace(")", "")
def strip_code(display_str): return display_str.split(" (")[0]
...
def build_demo_batch(region_name, cfg, include_hidden=True):
    ...
    oc_cycle = [0.5, 1.2, 1.8, 2.2, 2.8, 3.5, 4.5, 6.0]
    lat_mid, lon_mid = cfg["default_latlon"]
    i = 1
    for tax in taxa:
        for tex in textures:
            if region_name == "Florida" and tax == "S1" and tex != "T5": continue 
            if region_name == "Florida" and tax != "S1" and tex == "T5": continue
            oc = oc_cycle[(i - 1) % len(oc_cycle)]
            row = {
                "sample_id": f"{cfg['key']}_{i:03d}", "oc": oc, "peer_group_taxon": tax, "peer_group_texture": tex,
                "PRISM_tmea": round(float(cfg["temp_default"] + rng.uniform(-2, 2)), 1),
            }
            if "precip" in cfg["predictors"]: row["PRISM_ppt"] = round(float(cfg["precip_default"] + rng.uniform(-150, 150)), 0)
...
def render_single_sample(region_name, cfg, df, df_hist):
    ...
    with st.expander("⚙️ Site Inputs", expanded=True):
        ...
        with c1:
            ...
            if region_name == "Brazil":
                ...
                active_taxon_display = cfg["taxon_display_sibc"] if "SiBC" in br_tax_system else cfg["taxon_display"]
                if "SiBC" in br_tax_system: taxon_label = "Ordem / Subordem (SiBC)"
            else:
            ...
            # ── UNIFIED MASTER CROP SELECTION TIE-IN ──
            ...
            if cfg["has_histosol"]: hist_toggle = st.checkbox("🧬 This is an organic / Histosol soil (Muck, Peat)")
            else: hist_toggle = False
        ...
        with c2:
            ...
            if use_geo:
                ...
                if st.button("🌐 Fetch Climate Data", key=f"{k}_fetch"):
                    if not in_bounds(lat_in, lon_in, cfg):
                        ...
                        if res:
                            if "temp" in res: st.session_state[f"{k}_temp"] = float(np.clip(res["temp"], *cfg["temp_range"]))
                            if has_precip and "precip" in res: st.session_state[f"{k}_precip"] = float(np.clip(res["precip"], *cfg["precip_range"]))
                            st.success("Climate metric parameters downloaded successfully.")
            ...
            if current_indicator == "Soil Phosphorus (SMAF)":
                ...
                target_precip = st.slider("Mean Annual Precipitation (mm)", cfg["precip_range"][0], cfg["precip_range"][1], value=float(st.session_state.get(f"{k}_precip", cfg["precip_default"])), step=10.0, key=f"{k}_precip") if has_precip else None
    ...
    chosen_indicator = st.selectbox("Soil Health Indicators:", indicator_options, key=f"{cfg['key']}_indicator_shared")
    st.divider()

    col_l, col_r = st.columns([1, 2])

    # ════════════════════════════════════════════════════════════════════
    # BRANCH A: DYNAMIC SMAF PHOSPHORUS WORKFLOW
    # ════════════════════════════════════════════════════════════════════
    ...
    # ════════════════════════════════════════════════════════════════════
    # BRANCH B: DYNAMIC GAUSSIAN PH WORKFLOW (75 CROPS SYSTEM)
    # ════════════════════════════════════════════════════════════════════
        ...
        if crop_selected_name not in ph_benchmarks:
            ...
            with col_r:
                st.markdown(f"#### {crop_selected_name} pH Optimization Curve")
                ...
                fig_cdf.add_trace(go.Scatter(x=[ph_val], y=[score_ph / 100.0], mode="markers", marker=dict(color=color_ph, size=14, line=dict(color="white", width=2)), name="Your Field pH"))
    ...
    # ════════════════════════════════════════════════════════════════════
    # BRANCH C: ORIGINAL ORGANIC CARBON CODE
    # ════════════════════════════════════════════════════════════════════
    else:
        ...
        with col_l:
            ...
            if has_precip and target_precip is not None: climate_str += f" · {target_precip:.0f}mm"
            ...
            fig_gauge = go.Figure(go.Indicator(
                mode="gauge+number", value=round(score, 1), title={"text": gauge_title, "font": {"size": 13}},
                number={"suffix": "/100", "font": {"size": 38, "color": color}},
                gauge={
                    "axis": {"range": [0, 100], "tickwidth": 1, "tickcolor": "gray"},
                    "bar": {"color": color, "thickness": 0.28}, "bgcolor": "rgba(0,0,0,0)",
                    "steps": [
                        {"range": [0, 20], "color": "rgba(215,48,39,0.35)"},
                        {"range": [20, 40], "color": "rgba(244,109,67,0.35)"},
                        {"range": [40, 60], "color": "rgba(255,193,7,0.35)"},
            ...
            gap = tgt_oc - oc_val
            m1, m2 = st.columns(2)
            with m1: st.metric("Above peer median", f"{score - 50:+.1f} pts")
            with m2: st.metric(f"Gap to {target_pct}th pct", f"{abs(gap):.2f}% SOC", "✅ Exceeds target" if gap <= 0 else f"+{gap:.2f}% needed")
        ...
        with col_r:
            ...
            fig_cdf = go.Figure()
            fig_cdf.add_trace(go.Scatter(x=np.concatenate([x, x[::-1]]), y=np.concatenate([y_ucl, y_lcl[::-1]]), fill="toself", fillcolor="rgba(26,150,65,0.18)", line=dict(color="rgba(0,0,0,0)"), name="95% Credible Interval", hoverinfo="skip"))
            fig_cdf.add_trace(go.Scatter(x=x, y=y_mean, mode="lines", line=dict(color="#1a9641", width=2.5), name="Score Curve"))
            fig_cdf.add_trace(go.Scatter(x=[oc_val], y=[score / 100], mode="markers", marker=dict(color=color, size=14, symbol="circle", line=dict(color="white", width=2)), name="Your Site"))
            fig_cdf.add_trace(go.Scatter(x=[tgt_oc], y=[target_pct / 100], mode="markers", marker=dict(color="#0072B2", size=13, symbol="x-thin", line=dict(color="#0072B2", width=3)), name="Target"))
            
            fig_cdf.update_layout(xaxis_title="Soil Organic Carbon (%)", yaxis_title="SHAPE Score", yaxis=dict(range=[0, 1], tickformat=".0%"), xaxis=dict(range=[0, plot_max]), legend=dict(orientation="h", yanchor="bottom", y=1.02), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", height=400, margin=dict(l=10, r=10, t=40, b=10))
        ...
        with st.expander("⚙️ Field Parameters", expanded=True):
            cc1, cc2, cc3, cc4, cc5 = st.columns(5)
            field_area = cc1.number_input("Field area (acres)", 1.0, 100000.0, 100.0, 10.0, key=f"{k}_area")
            bulk_density = cc2.number_input("Bulk density (g/cm³)", 0.8, 2.0, 1.45, 0.05, key=f"{k}_bd")
            depth_cm = cc3.number_input("Sampling depth (cm)", 5, 100, 30, 5, key=f"{k}_depth")
            carbon_price = cc4.number_input("Carbon price ($/t CO₂e)", 1.0, 500.0, 25.0, 5.0, key=f"{k}_price")
            annual_rate = cc5.number_input("Annual SOC gain (%/yr)", 0.01, 2.0, 0.20, 0.05, key=f"{k}_rate")

        def soc_to_tc_per_acre(soc_pct, bd, depth): return (soc_pct / 100.0) * bd * depth * 10.0 * 0.4047
        C_RATIO = 3.667
        ...
        sc1.metric("Current C stock", f"{curr_tc_field:,.1f} t C")
        sc2.metric("Target C stock", f"{tgt_tc_field:,.1f} t C")
        sc3.metric("Sequestration gap", f"{gap_co2_field:,.1f} t CO₂e")
        sc4.metric("Potential value", f"${credit_value:,.0f}")
        sc5.metric("Years to Target", f"{years_to_tgt:.1f} yrs")
...
def render_batch_scoring(region_name, cfg, df, df_hist):
    ...
    template_cols = {
        "sample_id": ["Site_A", "Site_B"], "oc": [1.8, 2.5],
        "peer_group_taxon": [parse_code(cfg["taxon_display"][0]), parse_code(cfg["taxon_display"][1])],
        "peer_group_texture": list(set(cfg["texture_map"].values()))[:2], "PRISM_tmea": [cfg["temp_default"]] * 2,
    }
    if has_precip: template_cols["PRISM_ppt"] = [cfg["precip_default"]] * 2
    template = pd.DataFrame(template_cols)

    bcol1, bcol2 = st.columns(2)
    bcol1.download_button("⬇️ Download Template", data=template.to_csv(index=False).encode("utf-8"), file_name=f"SHAPE_{cfg['key']}_template.csv", mime="text/csv", key=f"{k}_template_btn")
    if bcol2.button("✨ Try Demo Data", key=f"{k}_demo_btn"): st.session_state[f"{k}_batch_df"] = build_demo_batch(region_name, cfg)

    uploaded = st.file_uploader("Upload your CSV", type="csv", key=f"{k}_uploader")
    if uploaded is not None:
        try: st.session_state[f"{k}_batch_df"] = pd.read_csv(uploaded)
    ...
    if batch is not None:
        ...
        for _, r in batch.iterrows():
            s = compute_score(float(r["oc"]), 0.0, 1.0)
            scores.append(round(s, 2)); labels.append(score_label(s))
        batch = batch.copy()
...
def render_how_to_use(region_name, cfg):
    ...
    st.markdown("1. Select your target soil region tab.\n2. Pick the appropriate indicator sub-group mapping toggle.\n3. Input field diagnostic data metrics to generate dynamic predictive function visuals.")

def render_region(region_name, cfg):
    mineral_df, hist_df = load_region_data(cfg)
    if mineral_df is None:
        st.error(f"Parameter file '{cfg['csv']}' missing.")
        return
    tab_single, tab_batch, tab_use = st.tabs(["🔬 Single Sample", "📊 Batch Scoring", "📖 How to Use"])
    with tab_single: render_single_sample(region_name, cfg, mineral_df, hist_df)
    with tab_batch: render_batch_scoring(region_name, cfg, mineral_df, hist_df)
    with tab_use: render_how_to_use(region_name, cfg)
...
for tab, (name, cfg) in zip(region_tabs, REGIONS.items()):
    with tab: render_region(name, cfg)
